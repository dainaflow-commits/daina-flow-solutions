...
export function ChatBot() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    {
      role: "assistant",
      content:
        "Oi! Eu sou a **Flow**, assistente virtual da Daina Flow ✨\n\nPosso te ajudar com dúvidas sobre serviços, prazos ou agendar uma conversa com a Larissa. O que você precisa hoje?",
    },
  ]);
  const [input, setInput] = useState("");
  const [streaming, setStreaming] = useState(false);
  const [siteContext, setSiteContext] = useState<{ services: string[]; whatsapp?: string; email?: string }>({ services: [] });
  const scrollRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    Promise.all([
      supabase.from("services").select("title").eq("active", true),
      ...
      setSiteContext({
        services: (svc.data ?? []).map((s) => s.title),
  ...
  async function captureLeadFromTranscript(history: Msg[]) {
    ...
    await supabase.from("leads").insert({
      name,
      email: email ?? "sem-email@chatbot.local",
      phone: phone ?? null,
    ...
    >
      ...
      >
        {typeof children === "string" ? (
          <div className="prose prose-sm max-w-none [&_p]:m-0 [&_p+p]:mt-2 [&_ul]:m-0 [&_ul]:pl-4 [&_strong]:font-semibold">
            <ReactMarkdown>{children.replace("[LEAD_CAPTURED]", "").trim()}</ReactMarkdown>
          </div>
        ) : children}
      </div>
    </motion.div>
