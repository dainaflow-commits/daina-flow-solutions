...
export const Route = createFileRoute("/dashboard/notificacoes")({
  ...
  component: () => <DashboardLayout><AdminNotifications /></DashboardLayout>,
});

interface Notif { id: string; title: string; message: string | null; link: string | null; read: boolean; created_at: string; }
interface Prefs {
  user_id: string;
...
function AdminNotifications() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setPrefs((pref as Prefs) ?? { user_id: user.id, ...DEFAULTS });
      setLoading(false);
    })();
  }, [user]);

  async function togglePref(key: keyof typeof DEFAULTS) {
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Configure quais alertas você quer receber.</p>
      </div>
        ...
        <section className="rounded-2xl border border-border bg-card p-6">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {([
              ...
              ["inapp_proposals", "Sino: propostas (aceite/recusa)"],
              ...
              <label key={k} className="flex cursor-pointer items-center justify-between rounded-xl border border-border bg-background px-4 py-3 text-sm">
                <span>{label}</span>
                <input type="checkbox" checked={(prefs as any)[k]} onChange={() => togglePref(k as any)} className="h-4 w-4 accent-primary" />
              </label>
            ))}
          </div>
        </section>
      ...
      <section className="rounded-2xl border border-border bg-card">
        <div className="border-b border-border px-6 py-4"><h2 className="font-display text-lg font-bold">Histórico</h2></div>
        {items.length === 0 ? (
